 ...
 *
 * 預設 hermetic：以 in-memory `ioredis-mock` 取代 BullMQ 連線，並 stub 每個 `@Processor`
 * （`KeywordAnalysisProcessor` / `TopicClusterProcessor` / `TrackingRefreshProcessor`）——否則
 * `@nestjs/bullmq` explorer 會起真 Worker 去 poll Redis：無 Redis 的 CI 噴 async ECONNREFUSED，
 * 而 ioredis-mock 下 BullMQ `moveToActive` 需 `cmsgpack`（mock 不支援）→ unhandled error 卡住 event loop
 * → beforeAll hook timeout。新增 `@Processor` 時務必在此一併 stub。
 *
