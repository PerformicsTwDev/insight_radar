 ...
 *   fallback（回空 collected，由 postProcess 補預設）**且入 needsReview**——降級即需覆核，使 job-level
 *   needsReview 完整收斂、run 正確標 partial（INV-6/AC-42.5，#684）。
...
export async function resilientChunk<R, I = string>(
  ...
  try {
    ...
    if (error instanceof LengthFinishReasonError) {
      if (chunk.length === 1) {
        // 拆到底（size 1）仍 length → 放棄該筆（postProcess 補預設），但**入 needsReview** 使 job-level
        // needsReview 完整收斂 → run 正確標 partial（非誤標 completed，INV-6/AC-42.5，#684）。
        return { collected: [], needsReview: [...chunk] };
      }
