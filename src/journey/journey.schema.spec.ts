import { JOURNEY_STAGES, journeyResponseFormat } from './journey.schema';
...
describe('journey json_schema (T12.5 / TC-69 部分)', () => {
  const rf = journeyResponseFormat();
  const schema = rf.json_schema.schema as Record<string, unknown>;

  it('is a strict json_schema response format named journey_classification', () => {
    expect(rf.type).toBe('json_schema');
    expect(rf.json_schema.strict).toBe(true);
    expect(rf.json_schema.name).toBe('journey_classification');
  });

  it('constrains stage to the seven journey enum values (single-label)', () => {
    ...
    const stage = item.properties.stage as { type: string; enum: string[] };
    // single-label：stage 是**純字串 enum**（非 array）——每字恰一 stage。
