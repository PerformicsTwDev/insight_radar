...
M14 抓下的 `ai_search_captures`（AI 回答全文 + 引用連結）在 M15 經 LLM 三線（品牌抽取 / 情緒 / 引用媒體分類）+ 純函式指標計算，落 `ai_answers` / `ai_cited_references` / `ai_visibility_metrics`，供讀取層 view（FR-44）呈現。分析層版本 `AI_VISIBILITY_SCHEMA_VERSION`（`v\d+`、預設 `v1`、bump 整批失效）為 M15 分析的**唯一** invalidation lever，**與抓取層 `AI_SEARCH_SCHEMA_VERSION` 分工**（分表互補、互不覆寫）。bump 分析版本亦改動 AI Search run 的 idempotency key（M15-R5/#687），故對**已完成** run 亦生效——同 keywords/channels/brandProfileId 再 POST 會產生新 run、重跑分析、rows 打新版本 tag（早期三線 per-prompt 版本 `BRAND_EXTRACT`/`SENTIMENT`/`MEDIA_CLASSIFY_PROMPT_VERSION` production 零消費、屬 documented no-op，已於 M15-R6/#688 移除）。
...
**症狀 → 處置**：`share of voice` 呈現 `—`/空 → 分母 0（範疇零品牌提及，正常，非 bug）；`exposure` 為 `—` → 範疇關鍵字 `avgMonthlySearches` 全 null（不補 0，正常）；某品牌 mentions 看似「偏高」→ 確認是否同則多次露出（不去重＝設計，非重複計數 bug）；改 prompt / schema 後舊資料仍生效 → bump `AI_VISIBILITY_SCHEMA_VERSION`（整批失效 + 對已完成 run 亦強制新 run 重跑，M15-R5/R6）。
