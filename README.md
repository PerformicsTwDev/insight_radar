...
- **API 契約**：[`docs/API.md`](./docs/API.md)（44 端點：`/health` + 4 auth + 11 keyword-analyses（含 M12 AI 洞察/購買歷程/自訂分類）+ 9 tracking-lists（M11）+ `/ai-ideation`（M12）+ `/views` + `/captures`（M13 ingestion）+ 5 brand-profiles（M14）+ 3 ai-search-analyses（M14 AI Search 抓取）；+ DTO + 錯誤信封 + 認證 + 同步 vs async job 取捨）
